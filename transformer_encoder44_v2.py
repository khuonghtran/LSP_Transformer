...
# *****************************************************************************************************************************************************
# layern=3; units=64; n_times=244; n_feature=1; n_head=4; is_batch=True; drop=0.1; mask_value = -9999.0;  active="exponential"; n_out=4
# L2=0; is_day_input=False; is_sensor=False; is_sensor_embed=False; is_xy=False 
# xy_n=2; is_reflectance=False; dense_layer_n=0; newunits=128
# inputs = trainx_transformer[:2,:,:] # no filled data
# https://www.tensorflow.org/text/tutorials/transformer
def get_transformer_new_att0_daily_withsensor(n_times=14, n_feature=2, n_out=9, layern=3, units=128, n_head=4, drop=0.1, is_batch=True, mask_value=-9999.0, active="sigmoid", 
        L2=0,is_day_input=False, is_sensor=False, is_sensor_embed=False, is_xy=False, xy_n=2, is_reflectance=True, dense_layer_n=0,newunits=128): # changed from exponential/linear/sigmoid softmax to sigmoid on Sep 19 2023
    """
    ...
    inputs = Input(shape=(n_times, n_feature+0+is_sensor+is_xy*xy_n,))  #   K changed on Sep 19 2023
    reg = None
    if L2>0:
    ...
    ## try turning this two lines on and off to see whether they make an impact 
    # attn_output, attn4 = MultiHeadAttention(key_dim=units // n_head, num_heads=n_head, kernel_regularizer=reg)(query=x, value=x, key=x,
        # return_attention_scores=True, attention_mask=padding_mask)
    ...
    ## *******************
    ## start to output 
    print('Is reflectance',is_reflectance)
    print('Activation functions is:',active)
    if is_reflectance:
        output = Dense(n_out, activation=active, kernel_regularizer=reg)(x)
    else:
        # newunits = units//2 
        ...
        output = Dense(n_out, activation=active, kernel_regularizer=reg)(x) # fix this bug on Mar 14, 2023

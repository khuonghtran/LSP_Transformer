...
def my_train_1schedule(model,train_datasetx,train_datasety,epochs=10,start_rate=0.001,loss=np.nan,per_epoch=100,split_epoch=4,option=0,decay=1e-5, 
        batch_size=32,validation_split=0.04,hold_epoch=0,reduce_epoch=True):
    ...
    model.compile(optimizer=optimizer, loss=loss, metrics=[tf.keras.metrics.RootMeanSquaredError()]) #Khuong changed Sep 21 2023
    ...
    if reduce_epoch:
        ...
        final_epochs = (epochs-30) if epochs>10 else epochs
    else:
        final_epochs = epochs
...
## testing LSPD accuacy 
def test_lsp_accuacy_old(model,input_images,y_test_raw,y_mins,y_maxs):
    y_pred_raw = model.predict(input_images)
    y_test_unnorm = (y_test_raw*(y_maxs-y_mins)+y_mins).astype('h') # Min/Max Normalization
    y_pred_unnorm = (y_pred_raw*(y_maxs-y_mins)+y_mins).astype('h') # Min/Max Normalization
    
    #Convert to doys
    doy3d = np.arange(244)
    doy3d_3bands = np.moveaxis(np.transpose(np.tile(doy3d, (y_test_unnorm.shape[0],4, 1))).astype('h'),-1,0)
    y_test= ((doy3d_3bands-y_test_unnorm/3-61)*3).reshape(-1,244*4).astype('h')
    y_pred= ((doy3d_3bands-y_pred_unnorm/3-61)*3).reshape(-1,244*4).astype('h')
            
    r=[]
    p_value=[]
    for i in range(y_test.shape[1]):
        pearson_r, pearson_p = stats.pearsonr(y_test[:,i],y_pred[:,i])
        r.append(pearson_r)
        p_value.append(pearson_p)
    r = np.array(r)
    p_value = np.array(p_value)
    mad = np.average(np.abs(y_test-y_pred),axis=0)
    rmse = np.sqrt(np.average(((y_test-y_pred).astype('f'))**2,axis=0))
    msb = np.average((y_pred-y_test),axis=0)
    #print("labels_test out!!!!!!!!!!!!!!!!!!!")
    return r,p_value,mad,rmse,msb,y_test,y_pred
    
## testing LSPD accuacy 
def test_lsp_accuacy(model,input_images,y_test_raw,y_mins,y_maxs):
    y_pred_raw = model.predict(input_images)
    y_test_unnorm = (y_test_raw*(y_maxs-y_mins)+y_mins).astype('h') # Min/Max Normalization #N x 244 x 4
    y_pred_unnorm = (y_pred_raw*(y_maxs-y_mins)+y_mins).astype('h') # Min/Max Normalization
    
    #Convert to doys
    doy3d = np.arange(244)
    doy3d_3bands = np.moveaxis(np.transpose(np.tile(doy3d, (y_test_unnorm.shape[0],4, 1))).astype('f'),-1,0) #N x 244 x 4
    y_test= np.nanmean((doy3d_3bands-y_test_unnorm/3-61)*3,axis=1).astype('h')
    y_test_std= np.nanstd((doy3d_3bands-y_test_unnorm/3-61)*3,axis=1).astype('h')
    y_pred= np.nanmean((doy3d_3bands-y_pred_unnorm/3-61)*3,axis=1).astype('h')
    y_pred_std= np.nanstd((doy3d_3bands-y_pred_unnorm/3-61)*3,axis=1).astype('h')
    y_pred2 = np.round(((doy3d_3bands-y_pred_unnorm/3-61)*3).reshape(-1,976),0)
    
    doy3d_3bands_masked=doy3d_3bands.copy()
    doy3d_3bands_masked[(np.tile(input_images,4))==-9999]=np.nan
    y_pred_masked= np.nanmean((doy3d_3bands_masked-y_pred_unnorm/3-61)*3,axis=1).astype('h')
    y_pred_std_masked= np.nanstd((doy3d_3bands_masked-y_pred_unnorm/3-61)*3,axis=1).astype('h')
    ...
    for i in range(y_test.shape[1]):
        #pearson_r, pearson_p = stats.pearsonr(y_test[:,i],y_pred[:,i])
        pearson_r, pearson_p = stats.pearsonr(y_test[:,i],y_pred_masked[:,i])
        r.append(pearson_r)
        p_value.append(pearson_p)
    r = np.array(r)
    p_value = np.array(p_value)
    mad = np.average(np.abs(y_test-y_pred_masked),axis=0)
    rmse = np.sqrt(np.average(((y_test-y_pred_masked).astype('f'))**2,axis=0))
    msb = np.average((y_pred_masked-y_test),axis=0)
    #print("labels_test out!!!!!!!!!!!!!!!!!!!")
    return r,p_value,mad,rmse,msb,y_test,y_test_std,y_pred,y_pred_std,y_pred_masked,y_pred_std_masked,y_pred2
